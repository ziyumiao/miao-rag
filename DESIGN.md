...
| 节点 | 功能 |
|------|------|
| 意图识别 | **并行输出**：情感分析、业务意图、实体提取、检索必要性判断、直接回答 |
| 追问节点 | 生成追问消息，等待用户回复后重入 |
| 检索节点 | 混合检索（BM25 + 向量）+ 知识图谱查询 |
| 判断节点 | 评估检索结果质量，决定下一步 |
| 视觉识别 | 调用多模态 LLM 识别轴体图片 |
...
```json
{
  ...
  "entities": {"keyboard_model": "K70", "brand": "Corsair"},
  "retrieval_needed": false,
  "answerable_directly": true,
  "direct_answer": "我们是卖键帽的，不卖键盘哦~需要给您推荐键帽吗？"
}
```

**检索必要性判断**：需要商品规格、兼容性信息、专业术语解释时设置 `retrieval_needed=true`；凭常识或 FAQ 就能回答时设置 `retrieval_needed=false` 并给出 `direct_answer`。闲聊和 FAQ 可覆盖的问题不走检索，节省 LLM 调用。

### 售后问题分层处理

售后问题不全部转人工。Agent 先尝试回答，需要操作权限时才转：

| 售后类型 | 知识库能否覆盖 | 处理方式 |
|----------|---------------|----------|
| 使用咨询（松动/打油/清洗） | ✅ FAQ 有答案 | Agent 直接回答 |
| 兼容性确认（买错了能不能用） | ✅ 兼容性判断 | Agent 回答 + 退换政策说明 |
| 退换货（要退/要换） | 部分（流程说明） | Agent 说明流程 → 客户确认 → 转人工执行 |
| 物流/发错/质量问题 | ❌ 需要人工权限 | 直接转人工 |

核心原则：**Agent 能回答的回答，需要操作权限的转人工**。
...
| P0.5 | 安全加固 | ✅ 完成 | API Key 认证、防注入、CORS、限流、RCTRF模板 |
| P1 | 知识冷启动 | ✅ 完成 | products.md, faq.md, keyboard_db.json |
| P2 | 清洗流水线 | ✅ 完成 | 规则引擎（7句式）+ LLM兜底 + JSONL导出 |
