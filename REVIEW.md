...
**分支**：master（HEAD: 046202d）  
**评分**：8.5 / 10（+1.5）  
**测试**：30 passed ✅

### 修复验证

三个严重问题全修复 ✅：PG 降级日志、use_pg 自动检测、API Key 默认值清空。建议项修复覆盖率约 70%。

### 新发现问题（已修复）

| # | 问题 | 修复 |
|---|------|------|
| N1 | RateLimitMiddleware 未排除 /health 路径 | 添加 EXCLUDED_PATHS 检查在限流逻辑前 |
| N2 | 全局 QPS 先 append 后检查导致可用数偏差 | 改为先检查后 append |
| N4 | dict.txt "原厂 3" 与 "原厂高度 5" 词频不一致 | 统一为 5 |

### 仍需 P3/P4 修复的已知项

| # | 问题 | 计划 |
|---|------|------|
| R14 | `filter_input` 未接线 | P4 接入 LLM 时在 `/qa/chat` 入口调用 |
| R15 | RCTRF 子层分隔不完整 | P4 调整 prompt_template.py |
| — | `secrets.compare_digest` | P3 补充 |
| — | 限流测试 flaky | P3 补 mock |
| — | 缺 `conftest.py` | P3 重构 |
| — | 缺 `[tool.mypy]` 配置 | P3 补充 |
